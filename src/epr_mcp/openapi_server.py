...
class EPROpenAPIHandler:
    ...
    async def handle_create_event(self, event_data: dict) -> dict:
        ...
        async with httpx.AsyncClient() as client:
            response = await client.post(f"{self.epr_url}/api/v1/events", json=event.as_dict_query(), headers=self.headers)
    ...
    async def handle_create_receiver(self, receiver_data: dict) -> dict:
        ...
        async with httpx.AsyncClient() as client:
            response = await client.post(f"{self.epr_url}/api/v1/receivers", json=receiver.as_dict_query(), headers=self.headers)
            if response.status_code == 201:
    ...
    async def handle_create_group(self, group_data: dict) -> dict:
        ...
        async with httpx.AsyncClient() as client:
            response = await client.post(f"{self.epr_url}/api/v1/groups", json=group.as_dict_query(), headers=self.headers)
            if response.status_code == 201:
